...
def get_good(driver):
    try:
        ...
        for good in good_list:
            good_name = good.find_element(By.CSS_SELECTOR,'.p-name em').text
            # good_name = good.find_element_by_css_selector('.p-name em').text
            # print(num)
            print(good_name)
            # num += 1
            with open('static/jdPName.txt', 'a', encoding='utf-8') as f:
                f.write(good_name)
        # 找到页面下一页点击
...
# num = 1
    try:
        ...
        wc = WordCloud(background_color='white',
                    ...
                    width=600,
                    height=600).generate(jieba_processig('static/baidu_stopwords.txt', 'static/jdPName.txt'))
        pUrl = "static/jdPName.png"
        wc.to_file(pUrl)
