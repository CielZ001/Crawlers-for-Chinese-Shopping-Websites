...
def parsePage(ilt, html):  # 对每一个获得的页面进行解析
    ...
    try:
        time.sleep(10)
        re1 = re.compile(r'\"nick\"\:\".*?\"')  # 编译商品名称正则表达式
        tlt = re1.findall(html)
        #plt = re.findall(r'\"view_price\"\:\"[\d\.]*\"', html)
        #tlt = re.findall(r'\"raw_title\"\:\".*?\"', html)
        for i in range(len(tlt)):
            brand = eval(tlt[i].split(':')[1])  # 去掉raw_title字段，只要名称部分
            ilt.append([brand])
...
def main():
    ...
    with open('static/tbBrand.txt', mode='w', encoding='utf-8') as f:
        f.write(ConvertStr)
    ...
    wc = WordCloud(background_color='white',
                   ...
                   width=600,
                   height=600).generate(jieba_processig('static/baidu_stopwords.txt', 'static/tbBrand.txt'))
    pUrl = "static/wordcloudTBBrand.png"
    wc.to_file(pUrl)
