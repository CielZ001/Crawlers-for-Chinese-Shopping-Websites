...
def parsePage(ilt, html):  # 对每一个获得的页面进行解析
    ...
    try:
        time.sleep(10)
        re1 = re.compile(r'\"raw_title\"\:\".*?\"')  # 编译商品名称正则表达式
...
def main():
    ...
    with open('static/tbPName.txt', mode='w', encoding='utf-8') as f:
        f.write(ConvertStr)
    ...
    wc = WordCloud(background_color='white',
                   ...
                   width=600,
                   height=600).generate(jieba_processig('static/baidu_stopwords.txt', 'static/tbPName.txt'))
    pUrl = "static/tbPName.png"
    wc.to_file(pUrl)
